...
def animate_attack(attacking_sprite, attacked_sprite, new_hp, callback):
    ATTACK_WITHDRAW_ANIMATION_DURATION = 0.2
    FLINCH_ANIMATION_DURATION = 1 # must be longer than 2 * withdraw dration
    UPDATE_HP_DURATION = 0.2

    def animate_update_target_hp():
        scale_x = (max(5, new_hp) if new_hp else 0) / 200 # must show some (minimum) hp
...
def animate_eat(sprite, new_hp, callback):
    ...
    update_hp_action = ScaleXTo(new_hp/200, EAT_ANIMATION_DURATION)
    end_action = cocos.actions.CallFunc(callback)
